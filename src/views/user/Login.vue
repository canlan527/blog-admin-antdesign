        ...
        <a-tab-pane key="tab1" :tab="$t('user.login.tab-login-credentials')">
          ...
          <a-row :gutter="16">
            ...
            <a-col class="gutter-row" :span="10">
              <div class="getCaptcha" v-html="svgCode" @click.stop.prevent="getCaptchaCode"></div>
            </a-col>
          </a-row>
        </a-tab-pane>
...
import { getSmsCaptcha, get2step, getCaptchaCode } from '@/api/login'
...
export default {
  ...
  methods: {
    ...
    handleVarificationCode(rule, value, callback) {
      // console.log(rule);
      // console.log(value);
      callback()
    },
    ...
    handleSubmit(e) {
      ...
      const validateFieldsKey =
        customActiveKey === 'tab1' ? ['loginId', 'loginPwd', 'captcha', 'remember'] : ['mobile', 'captcha']

      validateFields(validateFieldsKey, { force: true }, (err, values) => {
        if (!err) {
          const loginParams = { ...values }
          loginParams.remember = loginParams.remember ? 7 : 1
          // loginParams[!state.loginType ? 'email' : 'loginId'] = values.username
